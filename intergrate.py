...
j2_dict = {(item["home"], item["away"], item["runner"]): item for item in j2}

# 替换：如果 j2 中存在相同的 home, away 值，则替换 j1 中的字典
for i, item in enumerate(filtered_j1):
    home_away_pair = (item["home"], item["away"], item["runner"])
