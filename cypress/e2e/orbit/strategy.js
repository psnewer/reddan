...
export default class StrategyExecutor {
        ...
        if (params.event.score_home.length === set && params.event.score_away.length === set)
            if (params.bet.runner.includes(params.bet.home)){
                if (Number(params.event.score_home[set - 1]) >= 6)
                    return Number(params.event.score_home[set -1]) + 2 <= Number(params.event.score_away[set -1]);
            }
            else{
                if (Number(params.event.score_away[set -1]) >= 6)
                    return Number(params.event.score_away[set -1]) + 2 <= Number(params.event.score_home[set -1]);
            }
        return false
    }
    ...
    deltaIn(params,condition) {
        ...
        return ((Number(params.event.score_home[set -1]) - Number(params.event.score_away[set -1])) <= delta)
    }
    ...
    placeBet(params, condition) {
        ...
        cy.visit(params.bet.runner_url,{timeout:20000}).then(()=>{
            ...
            cy.contains('span', new RegExp(`^${runner}$`)).closest('div[data-market-id]')
            ...
            if (params.bet.strategy.params[condition].oth){
                if (params.bet.strategy.params[condition].side === 'BACK')
                    current_odds = bet.event.oth_back_odds;
                else
                    current_odds = bet.event.oth_lay_odds;
            }else{
                if (params.bet.strategy.params[condition].side === 'BACK')
                    current_odds = bet.event.back_odds;
                else
                    current_odds = bet.event.lay_odds;
            }
